...
ic,bc = auto.train(data,batch=bat,n_iters=5000,use_dropout=True,keep_prob=0.5,reg_weight=False,reg_lambda=0.0,model_name='conv_24.mod',display=False,noise=False,gradient='adam',learning_rate=0.000125)


print "Finished. Initial cost: ",ic," Final cost: ",bc

auto.load_model('conv_24.mod',session=session)
#data = data+m
auto.stop_dropout()
red_feat = auto.get_hidden(data,session=session)
out_feat = auto.get_output(data,session=session)
np.savetxt("red_24",red_feat)
np.savetxt("out_24",out_feat)
...
if(sys.argv[1]=='s'):
    for i in range(96):
        ...
        display.save(no[i],7,7,3,folder='./feat',name='out_24',index=i)